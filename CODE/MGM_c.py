...
def init(a,f,x_0,n_x,n_y,L):
    A    = a.copy()
    b    = f.copy()
    u    = x_0.copy()
    G_VC = Grid_VC(n_x,n_y,L)
    return A,b,u,G_VC
         

def presmoothing(S1_it,A,b,u,G):
    #print("Applying presmoothing, Level = " + str(G.L))   
    Mv = A.diagonal()
...
def PR(G):
    ...
    Pr = 2*Rt.T #sparse.kron(1/2*Iy.T, 1/2*Iy.T)
    #Rt = 2*Pr.T 
    #Pr= sparse.kron(1/2*Iy, 1/2*Ix)
    #print("R")
    #print(Rt.todense())
    #print("P")
    #print(Pr.todense())
...
def postsmoothing(S2_it,A,b,G,un):
    #print("Applying postsmoothing, Level = " + str(G.L))     
    Mv     = A.diagonal()
    M_Inv  = 0.2*diags(1/Mv)
    M_IS   = csc_matrix(M_Inv)   
    I_h    = sparse.eye(G.n)
...
def v_cicle(S1_it,S2_it,A,b,u,G, Nx_L):
    ...
    A,b,u,G_VC = init(A,b,u,G.n_x,G.n_y,G.L)   
    ...
    if G_VC.N_x < 20:
        ...
        e2h, L , Nx_L    = v_cicle(S1_it,S2_it, A_2h, r_2h, e_init,Grid_VC(G.N_x,G.N_y,G.L), Nx_L ) 
...
def MG(a,f,x_0,x_true,G, S1_it, S2_it, MG_it_max,tol):
    ...
    A,b,x,G_VC = init(a,f,x_0,G.Nx,G.Ny,0)   
    rr = 1
    MG_it = 0
    ...
    while (rr > tol) and (MG_it<MG_it_max):
        ...
        e_vec.append(re)
        Its +=1
    flops_iter = MG_op(Nx_L,S1_it,S2_it)
